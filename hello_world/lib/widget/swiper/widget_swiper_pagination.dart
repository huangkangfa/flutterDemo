...
abstract class NioBasePainter extends BasePainter {
  ...
  final int index;
  final Paint _paint;
  final double lineMin = 2;
  final double lineMax = 4;
  final double lineOffset = 8;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0, c = widget.count; i < c; ++i) {
      if (_shouldSkip(i)) {
        _paint.strokeWidth = lineMax;
        canvas.drawLine(new Offset(i * (size + space) + radius - lineOffset, radius),
            new Offset(i * (size + space) + radius + lineOffset, radius), _paint);
        _paint.strokeWidth = lineMin;
        continue;
      }
      canvas.drawLine(new Offset(i * (size + space) + radius - lineOffset, radius),
